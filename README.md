...
Określają sposób wyboru procesu do wykonania. Wybór algorytmu ma bezpośredni wpływ na wydajność i zachowanie systemu. Są to np.:
...
Proces jest niezależną jednostką wykonania programu, posiadającą własną przestrzeń adresową, co zapewnia izolację od innych procesów, Dzięki temu błędy w jednym procesie nie wpływają na drugi.

Wątki są lżejszymi jednostkami wykonania działającymi w obrębie jednego procesu. Współdzielą one pamięć, co znacząco przyśpiesza komunikację, ale jednocześnie zwiększa ryzyko wystąpienia problemów współbieżności. Z punktu widzenia programisty ocznacza to konieczność większej ostrożności przy operacjach na wspólnych danych.
...
Optymalizacji wydajności (Futex - Fast Userspace Mutex):
